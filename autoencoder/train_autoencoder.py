...
def get_Callbacks(epochs, model_name):
    ...
    checkpoint = ModelCheckpoint('models/weights/' + model_name + '_weights',
...
def AutoEncoder(input_length, compression_size):
    ...
    decoded = Dense(input_length, activation='linear')(dense_up1)
...
def RossNet_CAE_Denoiser(input_length, compression_size, transfer_learning=True):
    ...
    up1 = UpSampling1D(size=2)(max3)#(reshaped)
...
def train_Model(model_class, model_name, transfer_learning=True):
    ...
    batch_size = 128
    epochs = 50
    x_train = np.load('data/train/train_seismos_noise_3sigma.npy')
    ...
    # Experiment with [-1,1] or [0,1] data
    #x_train = rescale(x_train)
    #y_train = rescale(y_train)
    #x_test = rescale(x_test)
    #y_test = rescale(y_test)

    print('Scaling safety check:', x_train[0].min(), x_train[0].max())
    ...
    print(model_name)
    autoencoder, encoder, decoder = model_class(x_train.shape[1], 1250, transfer_learning=transfer_learning)
    train_hist = autoencoder.fit(x_train, y_train,
                validation_data=(x_test, y_test),
                verbose=2,
                batch_size=batch_size,
                epochs=epochs,
                callbacks=get_Callbacks(epochs, model_name))
    autoencoder.load_weights('models/weights/' + model_name + '_weights')
    autoencoder.save('models/' + model_name + '.h5')
    #encoder.save('encoder.h5')
    #decoder.save('decoder.h5')
    np.savez('models/train_logs/' + model_name + '_train_log', loss=train_hist.history['loss'], val_loss=train_hist.history['val_loss'])

train_Model(RossNet_CAE_Denoiser, 'cae_denoiser_3sigma_mae_transfer_linear', transfer_learning=True)
