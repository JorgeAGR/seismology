...
! Variables
  ...
  REAL(KIND=kd) :: slw1,slw2,slwi,sphcor,pick1,pick2,SdStime
  ...
  CHARACTER(len=300) :: infile1,filename
...
pt1 = (pick1+400+(2*sample))/sample
pt2 = (pick2+400+(2*sample))/sample

!WRITE(filename,'(a,i4.4,a)') "jnk",bs,".txt"
!  OPEN(UNIT=222,FILE=filename,POSITION='APPEND')
!         DO j=pt1,pt2
!            WRITE(222,*) (j-1)*sample,vespa(i,j),slow
!         END DO
!  CLOSE(UNIT=222)

!Get maximum for each boot in window

pt3     = (MAXLOC(vespa(i,pt1:pt2),DIM=1))+pt1
SdStime = (REAL(pt3-1)*sample)-400
OPEN(UNIT=123,FILE="SdStimes.dat",POSITION='APPEND')
write(123,*) SdStime
CLOSE(UNIT=123)
