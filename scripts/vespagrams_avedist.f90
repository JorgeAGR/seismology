...
OPEN(UNIT=111,FILE="vespa.txt",STATUS='REPLACE')
...
!OLD      sphcor = (180_kd/pi)*(1_kd-COS((dist_ref-dist_st)*pi/180_kd))
    dist_st = seismodata(j,11)  
...
!      END IF
      ...
      IF ( (N1 < 0) ) THEN
         ...
         vespa(i,1:(inpts-ABS(N1))) = vespa(i,1:(inpts-ABS(N1))) +  SIGN((ABS(seismos(j,ABS(N1+1):inpts))**(1./root)),&
              (seismos(j,ABS(N1+1):inpts)))
      ELSE IF ( (N1 > 0) ) THEN
         vespa(i,(N1+1):inpts) = vespa(i,(N1+1):inpts) + SIGN((ABS(seismos(j,1:(inpts-N1)))**(1./root)),&
              (seismos(j,1:(inpts-N1))))
      ELSE IF ( (N1 == 0) ) THEN
         vespa(i,1:inpts) = vespa(i,1:inpts) + SIGN((ABS(seismos(j,1:inpts))**(1./root)),&
              (seismos(j,1:inpts)))
      END IF
